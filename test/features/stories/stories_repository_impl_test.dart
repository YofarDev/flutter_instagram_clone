...
void main() {
  ...
  group('createStory', () {
    test('returns Right(null) on success', () async {
      when(
        () => ds.createStory(filePath: '/tmp/s.jpg'),
      ).thenAnswer((_) async {});

      final Either<Failure, void> result = await repo.createStory(
        filePath: '/tmp/s.jpg',
      );
  ...
  group('fetchViewedStoryIds', () {
    test('returns Right with the viewed ids from datasource', () async {
      when(
        () => ds.fetchViewedStoryIds(storyIds: <String>['s1', 's2']),
      ).thenAnswer((_) async => <String>{'s1'});

      final Either<Failure, Set<String>> result = await repo
          .fetchViewedStoryIds(storyIds: <String>['s1', 's2']);
    ...
    test('returns Left when datasource throws', () async {
      when(
        () => ds.fetchViewedStoryIds(storyIds: <String>['s1']),
      ).thenThrow(Exception('reads failed'));

      final Either<Failure, Set<String>> result = await repo
          .fetchViewedStoryIds(storyIds: <String>['s1']);
